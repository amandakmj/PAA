...
int main(int argc, char *argv[]) {
    ...
    //Aqui
    int numVetores;
if (fscanf(input, "%d", &numVetores) != 1 || numVetores <= 0) {
    printf("Erro ao ler número de vetores.\n");
    fclose(input);
    return 1;
}

// Aloca memória para os vetores dinamicamente
Vetor *vetores = (Vetor*)malloc(numVetores * sizeof(Vetor));
if (vetores == NULL) {
    ...
    for (int i = 0; i < numVetores; i++) {
        ...
        //Aqui
        int *temporaria = (int*)malloc(vetores[i].tamanho * sizeof(int));
if (temporaria == NULL) {
    ...
    return 1;
        ...
        LA(temporaria, 0, tamanho - 1, &swaps, &calls);
        fprintf(output, ",LA(%d)", swaps + calls);  // Corrigido aqui
        ...
        HM(temporaria, 0, tamanho - 1, &swaps, &calls);  // Corrigido aqui
        fprintf(output, ",HM(%d)", swaps + calls);  // Corrigido aqui
        ...
        HA(temporaria, 0, tamanho - 1, &swaps, &calls);
        fprintf(output, ",HA(%d)", swaps + calls);  // Corrigido aqui
